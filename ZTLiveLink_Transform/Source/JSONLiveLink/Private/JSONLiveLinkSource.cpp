...
void FJSONLiveLinkSource::HandleReceivedData(TSharedPtr<TArray<uint8>, ESPMode::ThreadSafe> ReceivedData)
{
	...
	if (FJsonSerializer::Deserialize(Reader, JsonObject))
	{
		for (TPair<FString, TSharedPtr<FJsonValue>> &JsonField : JsonObject->Values)
		{
			...
			const TSharedPtr<FJsonObject> TransformObject = JsonField.Value->AsObject();
			...
			if (!Stopping)
			{
				...
				if (TransformObject->TryGetArrayField(TEXT("UserData"), DataArray) && DataArray->Num() == 10)
				{
					...
					// double Data6 = (*DataArray)[6]->AsNumber(); //not in use
					// double Data7 = (*DataArray)[7]->AsNumber(); //not in use
					// double Data8 = (*DataArray)[8]->AsNumber(); //not in use
					// double Data9 = (*DataArray)[9]->AsNumber(); //not in use
				...
				TransformFrameData.Transform = FTransform(Quaternion, Translation, Scale);
