	...
	if (FJsonSerializer::Deserialize(Reader, JsonObject))
	{
		for (TPair<FString, TSharedPtr<FJsonValue>> &JsonField : JsonObject->Values)
		{
			...
			if (!Stopping)
			{
				...
				double User_FocalLength;
				double User_FocusDistance;
				double User_Aperture;
				...
				if (CameraObject->TryGetArrayField(TEXT("UserData"), DataArray) && DataArray->Num() == 10)
				{
					...
					User_FocalLength = Data6;
					User_FocusDistance = Data7;
					User_Aperture = Data8;
				}
				else
				{
				...
				CameraFrameData.FocalLength = User_FocalLength;
				CameraFrameData.FocusDistance = User_FocusDistance;
				CameraFrameData.Aperture = User_Aperture;
